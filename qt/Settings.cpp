...
void pigaleWindow::LoadSettings()
  ...
  InputFileName = setting.value("TgfFile/input",QString(PACKAGE_PATH)+ QDir::separator()
                                +"tgf"+QDir::separator()+ "a.tgf").toString();
  OutputFileName = setting.value("TgfFile/output",InputFileName).toString();
  DirFileDoc = setting.value("dorDoc",QString(PACKAGE_PATH)+QDir::separator()+"Doc").toString();
  setting.endGroup();
  //geometry
  setting.beginGroup("geometry");
  ...
  randomSetSeed() = (long) setting.value("randomSeed",1).toInt();
  setting.endGroup();

...
class RnEmbedding {
 ...
 public:
  ...
  RnEmbedding(int nv, int dm, int faces): dmax(dm),facets(faces),n(nv),
    Space(1,dm),i1(1),i2(2),i3(3), Coord3(0,n)
  ...
  double rx(tvertex i) const {return Coord3[i].x();}
  double ry(tvertex i) const {return Coord3[i].y();}
  double rz(tvertex i) const {return Coord3[i].z();}

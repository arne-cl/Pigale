  ...
  for(i = 1;i <= 2*m;++i)
      ...
      if(Dos.IsTree(ee))
          ...
          if(Dos.vin[b] == Dos.vin[es] || Dos.vin[b] == Dos.vin[-es])
              ...
              h[es] = Max((double)level[Dos.vin[b]] + vpush/polar_beta,h[es]); 
          }
          ...
          if (es==0)
              ...
              h[es] = Max(h[ee] + epush/polar_beta,h[es]);
          else
              h[es]=Max(h[ee] + extpush/polar_beta,h[es]);
          }
      }
  return h[0];
  }
...
double pigaleComputeAngles(TopologicalGraph &G, svector<int> &level, int
  ...
  for (v=2;v<=n;v++)
      polar_beta=Max(polar_beta,angle[v]/(2*acos((double)level[v]/(level[v]+0.5))));
  polar_beta=Max(polar_beta,angle[1]/(2*PI));
  int x;

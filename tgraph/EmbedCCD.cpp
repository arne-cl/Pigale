...
static void CountParents(TopologicalGraph &G,  const SchnyderWood &SW, 
  ...
  if (root == SW.GetRoot(c))
    assert(SW.GetFirstChildren(root,c) != 0);
  else 
    if (SW.IsRoot(root))
      return;


  for (v = SW.GetFirstChildren(root,c) ; v != 0; v = SW.GetNextChildren(v,c)) {
...
{ 
  ...
  // perfom cw-flips
  /*  cout << "DeltaSarpCW " << SW.CountDeltaSharpCW() << endl;
  cout << "DeltaSarpCCW " << SW.CountDeltaSharpCCW() << endl;
  cout << "DeltaSmoothCW " << SW.CountDeltaSmoothCW() << endl;
  cout << "DeltaSmoothCCW " << SW.CountDeltaSmoothCCW() << endl;*/
  int nb_mergesB = 0, nb_mergesR = 0, nb_mergesG = 0;
  if (compact) {
  ForAllVertices(v,G) {
    if (SW.cw_merge(v,Blue)) {
      cout << "merge Blue " << v << endl;
      nb_mergesB++;
    }
    if (SW.cw_merge(v,Green)) {
      cout << "merge Green " << v << endl;
      nb_mergesG++;
    }
    if (SW.cw_merge(v,Red)) {
      cout << "merge Red " << v << endl;
      nb_mergesR++;
    }
  }
  cout << "nb_merges  " << nb_mergesR + nb_mergesB + nb_mergesG << endl;
  //Check the number of flips !!!
  /*  cout << "DeltaSarpCW " << SW.CountDeltaSharpCW() << endl;
  cout << "DeltaSarpCCW " << SW.CountDeltaSharpCCW() << endl;
  cout << "DeltaSmoothCW " << SW.CountDeltaSmoothCW() << endl;
  cout << "DeltaSmoothCCW " << SW.CountDeltaSmoothCCW() << endl;*/
  ...
  for (b =G.cir(-FirstBrin); b != FirstBrin; b= G.cir[-b]) 
    nb_ext_vertices++;
  //  cout << "nb_ext = " << nb_ext_vertices << endl;
  assert(nb_mergesR + nb_mergesB + nb_mergesG  - (nb_faces -G.nv() + DeltaSmoothCCW_after)== 0);
  //  assert (nb_mergesR + nb_mergesB + nb_mergesG == nb_faces -G.nv() - (nb_ext_vertices -3) + DeltaSmoothCCW_after);
  }
  // face counting
  ...
  ForAllEdges(e,G0) {
    ...
    ForAllVertices(v,G) {
      /*    int nb_verticesB = SumDescB_R[v] + SumDescB_G[v] - DescendantsB[v];
    int nb_edgesB = 3* nb_verticesB  - DistanceRootR[v] - DistanceRootG[v] - SumDoubleB_G[v] -SumDoubleB_R[v] + DoubleEdgeDescendantsB[v] - 4;
    int nb_facesB = 2  - nb_verticesB + nb_edgesB;*/
    ...
    /*
      cout << "BB sumDoubleB_G " << SumDoubleB_G[v] << " sumDoubleB_R " << SumDoubleB_R[v] <<  " doubleEV " << DoubleEdgeDescendantsB[v] <<  " total " << SumDoubleB_G[v] + SumDoubleB_R[v] -  DoubleEdgeDescendantsB[v] << endl;
      cout << "GG sumDoubleG_R " << SumDoubleG_R[v] << " sumDoubleG_B " << SumDoubleG_B[v] <<  " doubleEV " << DoubleEdgeDescendantsG[v]  <<  " total " << SumDoubleG_B[v] + SumDoubleG_R[v] -  DoubleEdgeDescendantsG[v] << endl;
      cout << "RR sumDoubleR_B " << SumDoubleR_B[v] << " sumDoubleR_G " << SumDoubleR_G[v] <<  " doubleEV " << DoubleEdgeDescendantsR[v]  <<  " total " << SumDoubleR_G[v] + SumDoubleR_B[v] -  DoubleEdgeDescendantsR[v] << endl;
      cout << "totaux -----" << SumDoubleB_G[v] + SumDoubleB_R[v] -  DoubleEdgeDescendantsB[v] + SumDoubleG_B[v] + SumDoubleG_R[v] -  DoubleEdgeDescendantsG[v] + SumDoubleR_G[v] + SumDoubleR_B[v] -  DoubleEdgeDescendantsR[v] << endl;
    */
    x[v] = nb_facesG;
    y[v] = nb_facesR;

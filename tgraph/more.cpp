...
void Destroy(_svector &x) {delete (char *)x.buff;}
void Resize(_svector &x, int a, int b) {x.resize(a,b);}
...
void Build(tstring &x) {x.p = new tstring::srep;
      x.p->s = new char[1];
      x.p->s[0] = '\0';
      x.p->n=1;
      }
void Destroy(tstring &x) {x.delref();}
void Cast(const char *const str, tstring &x) {
      x.delref();
      x.p = new tstring::srep;
      x.p->n=1;
      x.p->s = new char[strlen(str) + 1];
      strcpy(x.p->s,str);
      x.p->s[strlen(str)] = '\0';
      }
void Copy(const tstring &src, tstring &dest) { dest=src; }
void Copy(const _svector &src, _svector &dest) { dest=src; }

void Clear(GraphContainer &x) {x.clear();}
void Setsize(GraphContainer &gc, int n, int m) {gc.setsize(n,m);}
int SaveGraphTgf(GraphAccess& G,tstring filename,int tag);
void SaveIt(GraphContainer &x) {GraphAccess ga(x); SaveGraphTgf(ga,tstring("cast.tgf"),1);}
